...
public class WildPokemonBattleScreen extends AbstractScreen {
    ...
    private enum GameState { INITIAL, PLAYER_OPTIONS, ATTACK_OPTIONS, BAG_OPTIONS, RUNNING, SWITCHING, OPPONENT_TURN,WAITING_FOR_PLAYER_CONFIRMATION,WAITING_FOR_OPPONENT_CONFIRMATION,CAPTURED_POKEMON,WIN,LOSS,USE_POKEBALL,USE_POTION }
    private GameState currentState;
    private boolean playerSwitched = false;

    public WildPokemonBattleScreen(GameApp app, Pokemon opponentPokemon) {
        super(app);
        battleTheme = Gdx.audio.newMusic(Gdx.files.internal("resources/Music/battleTheme.mp3"));
        battleTheme.setLooping(true);
        battleTheme.setVolume(0.1f);
    ...
    public void handleDialogue() {
        ...
        switch (currentState) {
            case INITIAL:
                setDialogue("A wild " + opponentPokemon.getName() + " appeared!");
            ...
            case OPPONENT_TURN:
                ...
                if(playerPokemon.isFainted()){
                    if(playerSwitched = false && GameContext.getInstance().getGamelevel() > 1){
                        playerSwitched = true;
            ...
            case CAPTURED_POKEMON:
                ...
                GameContext.getInstance().checkLevel();
                if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
                    battleTheme.stop();
                    GameContext.getInstance().setState(ExploringState.getOverworldState());
                    break;
    ...
    public void render(float delta) {
        batch.begin();
        battleTheme.play();
        Texture background = new Texture(Gdx.files.internal("resources/Battle/BattleScreenWild.png"));
