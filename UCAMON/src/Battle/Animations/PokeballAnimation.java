...
public class PokeballAnimation extends BattleAnimation{
    ...
    public void initialize(AssetManager assetManager, TweenManager tweenManager) {
         ...
         */
        Tween.to(pokeballSprite, BattleSpriteAccesor.X, pokeballOpen)
                .target(0f)
                .ease(Linear.INOUT)
                .start(tweenManager);
        Tween.to(pokeballSprite, BattleSpriteAccesor.Y, 0.25f)
                .target(1f)
                .ease(Sine.OUT)
                .start(tweenManager);
        Tween.to(pokeballSprite, BattleSpriteAccesor.Y, 0.25f)
         ...
         */
        Tween.to(pokeballSprite, BattleSpriteAccesor.ROTATION, 0.5f)
                .target(2f*360f)
                .ease(Linear.INOUT)
                .start(tweenManager);

        Tween.to(pokeballSprite, BattleSpriteAccesor.ALPHA, 0.25f)
                .target(0f)
                .ease(Linear.INOUT)
                .delay(0.7f)
                .start(tweenManager);
        ...
        Tween.to(this, BattleAnimationAccesor.PRIMARY_WIDTH, 0.5f)
                .target(1f)
                .delay(pokeballOpen)
                .start(tweenManager);
        Tween.to(this, BattleAnimationAccesor.PRIMARY_HEIGHT, 0.5f)
                .target(1f)
                .delay(pokeballOpen)
                .start(tweenManager);
        ...
        Tween.to(this, BattleAnimationAccesor.PRIMARY_MASK_AMOUNT, 1f)
                .target(0f)
                .ease(Linear.INOUT)
    ...
    private void addEffectSprite(Animation anim, float endX, float endY, TweenManager tweenManager) {
        ...
        Tween.to(effectSprite, BattleSpriteAccesor.ALPHA, 0f)
                .target(1f)
                .delay(pokeballOpen)
                .start(tweenManager);

        Tween.to(effectSprite, BattleSpriteAccesor.Y, 1f)
                .target(endY)
                .ease(Linear.INOUT)
                .delay(pokeballOpen)
                .start(tweenManager);

        Tween.to(effectSprite, BattleSpriteAccesor.X, 1f)
                .target(endX)
                .ease(Linear.INOUT)
                .delay(pokeballOpen)
                .start(tweenManager);
    ...
    public Texture getWhiteMask() {
        return whiteMask;
    }

    public void setWhiteMask(Texture whiteMask) {
